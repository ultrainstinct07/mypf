      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-dark via-dark to-dark-lighter border-b border-white/5">
        ...
        <div className="container-custom relative z-10 pt-28 pb-12">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="flex flex-wrap gap-2 mb-4">
                {frontmatter.tags?.map((tag: string) => (
                  <span
                    key={tag}
                    className="px-3 py-1 bg-cyan/20 text-cyan text-sm font-semibold rounded-full border border-cyan/30"
                  >
                    {tag}
                  </span>
                ))}
            ...
            <div className="relative aspect-video rounded-2xl overflow-hidden border border-cyan/20 glow-cyan">
              <Image
        ...
        <div className="container-custom py-8">
          ...
          <div className="flex flex-wrap gap-3">
            {frontmatter.techStack?.map((tech: string) => (
              <span
                key={tech}
                className="px-4 py-2 bg-dark border border-white/10 text-gray-300 rounded-lg text-sm font-medium hover:border-cyan/30 transition-colors"
              >
                {tech}
              </span>
            ))}
